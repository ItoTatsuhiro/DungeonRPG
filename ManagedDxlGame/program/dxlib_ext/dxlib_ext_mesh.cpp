...
namespace dxe {
	...
	Shared<Mesh> Mesh::CreateFromTypeDesc(dxe::Mesh::eShapeType shpe_type, dxe::MeshDesc* desc) noexcept {
		...
		case dxe::Mesh::eShapeType::BOX:
		{
			...
			return CreateBoxMV(d->size_, d->div_w_, d->div_h_, d->is_left_cycle_);
		}
	...
	void Mesh::calcVertexNoraml() {
		...
		for (int i = 0; i < idxs_.size(); i += 3) {
			tnl::Vector3 p1 = ToTnlVector3(vtxs_[idxs_[i + 0]].pos);
			tnl::Vector3 p2 = ToTnlVector3(vtxs_[idxs_[i + 1]].pos);
			tnl::Vector3 p3 = ToTnlVector3(vtxs_[idxs_[i + 2]].pos);
			...
			tnl::Vector3 sumv;
			DxLib::VECTOR nml;
			sumv = ToTnlVector3(vtxs_[idxs_[i + 0]].norm);
			sumv += tnl::Vector3::Cross(v2, v1);
			nml = ToDxlibVector3(sumv);
			vtxs_[idxs_[i + 0]].norm = nml;

			sumv = ToTnlVector3(vtxs_[idxs_[i + 1]].norm);
			sumv += tnl::Vector3::Cross(v2, v1);
			nml = ToDxlibVector3(sumv);
			vtxs_[idxs_[i + 1]].norm = nml;

			sumv = ToTnlVector3(vtxs_[idxs_[i + 2]].norm);
			sumv += tnl::Vector3::Cross(v2, v1);
			nml = ToDxlibVector3(sumv);
		...
		for (int i = 0; i < vtxs_.size(); ++i) {
			tnl::Vector3 n = ToTnlVector3(vtxs_[i].norm);
			n.normalize();
			vtxs_[i].norm = ToDxlibVector3(n);
		}
