...
BattleEnemyMelee::BattleEnemyMelee(tnl::Vector3 startPos, float objSize, std::string fileName)
...
// クラスの生成用の関数
std::shared_ptr<BattleEnemyMelee> BattleEnemyMelee::Create(tnl::Vector3 startPos, float objSize, std::string fileName) {

	// クラスのインスタンスを生成
	std::shared_ptr<BattleEnemyMelee> instance(new BattleEnemyMelee(startPos, objSize, fileName));
...
// 描画用の関数
void BattleEnemyMelee::draw(std::shared_ptr< dxe::Camera > camera) {
